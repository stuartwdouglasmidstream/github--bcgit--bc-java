...
if [ -n "" ]; then
  echo "Creating tag 1.71 to match enforced version"
  git tag -m 1.71 -a 1.71 || true
else
  echo "Enforce version not set, recreating original tag r1rv71"
  git tag -m r1rv71 -a r1rv71 || true
fi
